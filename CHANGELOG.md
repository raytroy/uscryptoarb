...
### Changed
- Renamed `src/uscryptoarb/calculation/types.py` -> `src/uscryptoarb/calculation/calc_types.py` to avoid future collision risk and improve AI-assisted module resolution.
- Renamed `src/uscryptoarb/connectors/base.py` -> `src/uscryptoarb/connectors/connector_base.py` for explicit connector scope naming.
- Renamed `src/uscryptoarb/venues/symbols.py` -> `src/uscryptoarb/venues/symbol_translator.py` to disambiguate from connector-internal `symbols.py` modules.
- Renamed `src/uscryptoarb/strategy/scanner.py` -> `src/uscryptoarb/strategy/trade_finder.py` to remove cross-package `scanner.py` collision.
- Renamed `src/uscryptoarb/orchestration/scanner.py` -> `src/uscryptoarb/orchestration/scan_loop.py` to remove cross-package `scanner.py` collision.
- Renamed tests for parity with module moves: `test_scanner.py` -> `test_trade_finder.py`, `test_scanner.py` -> `test_scan_loop.py`, and `test_base_connector.py` -> `test_connector_base.py`.
...
- `orchestration/scan_loop.py`: Gemini connector wired into create_connectors() factory
...
- `connectors/connector_base.py`: `BaseAsyncConnector` ABC with shared constructor, venue property, and `_fetch_with_retry()` retry/backoff/rate-limit logic (DEC-018)
- `venues/symbol_translator.py`: `create_translator()` factory function for validated SymbolTranslator construction
...
- `tests/unit/test_connectors/test_connector_base.py`: 13 tests for shared retry logic (timeout, 5xx, 429, 404, exhausted retries, rate limiting, header/param passthrough)
- **Orchestration layer** (`src/uscryptoarb/orchestration/`) — imperative shell wiring pure pipeline to real I/O
- **Verbose spread logging** in `orchestration/scan_loop.py` — per-pair bid/ask and best raw spread logged on every scan cycle for pipeline diagnostics
...
- **Calculation layer** (`src/uscryptoarb/calculation/`) — pure math layer for Type-2 arbitrage detection
  - `calc_types.py`: TradingFeeRate, WithdrawalFee, TradingAccuracy, FeeSchedule, ArbLeg, ArbOpportunity dataclasses
...
- **Strategy layer** (`src/uscryptoarb/strategy/`) — pure trade selection logic for Type-2 arbitrage detection
  ...
  - `trade_finder.py`: filter_valid_exchanges, find_trades_to_execute — top-level scan pipeline
...
- `connectors/connector_base.py`: `ExchangeConnector` Protocol defining the interface for all exchange connectors
...
- `venues/symbol_translator.py`: Added `to_canonical()` reverse lookup to SymbolTranslator
...
### Fixed
- `connectors/connector_base.py`: Unified 429 (rate limit) retry handling — Kraken connector previously did not retry on HTTP 429, now both connectors retry via shared logic (DEC-018)
...
- `strategy/trade_finder.py`: Removed redundant `sort_opportunities()` call in `find_trades_to_execute()` — `select_trade()` already sorts internally (Coding Rule 10.1)
